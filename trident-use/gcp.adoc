...
| `apiRegion` | Astra TridentがCloud Volumes Service ボリュームを作成するGoogle Cloudリージョン。複数リージョンのKubernetesクラスタを作成する場合は、に作成されたボリューム `apiRegion` 複数のGoogle Cloudリージョンのノードでスケジュールされたワークロードで使用できます。リージョン間トラフィックは追加コストを発生させます。 |  
...
| `serviceLevel` | 新しいボリュームのCVS -パフォーマンスレベルまたはCVSサービスレベル。CVS -パフォーマンスの値はです `standard`、 `premium`または `extreme`。CVSの値はです `standardsw` または `zoneredundantstandardsw`。 | CVS -パフォーマンスのデフォルトは「Standard」です。CVSのデフォルトは"standardsw"です。 
